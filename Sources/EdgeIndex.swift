...
//  Sphere2Go
//
//  Created by Axel Huesemann on 3/20/17.
...
class Iterator {
  ...
  var currentIndexInCandidates: Int
...
typealias CellEdgeMultimap = [CellId: Int] // TODO: multimap?
...
class S2EdgeIndex {
  ...
  var mapping: CellEdgeMultimap
  
  // No cell strictly below this level appears in mapping.  Initially leaf
  // level, that's the minimum level at which we will ever look for test edges.
  var minimum_s2_level_used: Int
  
  // Has the index been computed already?
  var indexComputed: Bool
  ...
  func reset() {
    minimum_s2_level_used = CellId.maxLevel
    indexComputed = false
    queryCount = 0
    mapping.removeAll()
  }
  ...
  // been computed.
  func computeIndex() {
    assert(!indexComputed)
    for i in 0..<num_edges {
//      S2Point from, to
      var cover = [CellId]()
      let level = getCovering(a: edge_from(i), b: edge_to(i), thicken_edge: true, result: &cover)
      minimum_s2_level_used = min(minimum_s2_level_used, level)
      for cellId in cover {
        mapping[cellId] = i
      }
    }
  ...
  var isIndexComputed: Bool { return indexComputed }
  ...
  var num_edges: Int { return 0 }
  func edge_from(index: Int) -> S2Point? { return nil }
  func edge_to(index: Int) -> S2Point? { return nil }
  
  // Appends to result all edge references in the map that cross the
  // query edge, and possibly some more.
  func findCandidateCrossings(a: S2Point, b: S2Point, result: [Int]) {}
  ...
  func insert(a: S2Point, b: S2Point, reference: Int) {}
  ...
  // the covered edge.
  func getCovering(a: S2Point, b: S2Point, thicken_edge: Bool, result: inout [CellId]) {}
  ...
  static func getEdgesInParentCells(cover: [CellId], mapping: CellEdgeMultimap, minimum_s2_level_used: Int,candidate_crossings: inout [Int]) {}
  ...
  static func getEdgesInChildrenCells(a: S2Point, b: S2Point, cover: [CellId], mapping: CellEdgeMultimap, candidate_crossings: [Int]) {
  }
